...
- Le script boucle jusqu'à `shortfall = 0`, puis s'arrête. Garde-fous : `IG_REFILL_MAX_PASSES` (12), `IG_REFILL_MAX_MINUTES` (40) et `IG_REFILL_QUOTA_FLOOR` — plancher de réserve mensuelle du fournisseur. **Sans valeur, le plancher est proportionnel au plan : 10 % du plafond (min 5).** L'ancien défaut fixe de 1500 était pensé pour le plan Pro (15 000/mois) : en gratuit (150/mois) il était TOUJOURS au-dessus du quota, la boucle s'auto-désactivait à la passe 0 et la sélection du matin restait vide (constat 02/08/2026 — c'était ça, le bouton à cliquer).
- **Plancher « sans site » (migration 028, 07/08/2026).** `ig_accounts.no_site_min` fixe le nombre MINIMUM de profils sans site dans la journée (défaut **50** = toute la journée). Réglable depuis le cockpit, au-dessus de la liste du jour. Les créneaux que le vivier sans-site ne peut pas remplir **restent vides** — c'est ce manque qui relance cette boucle. Conséquence directe sur le budget : en restreignant la sélection à une partie du vivier, `shortfall` reste positif plus longtemps et la boucle consomme plus de requêtes. Si ça devient trop cher, baisser le plancher (30-35) suffit — pas de déploiement. L'alerte Telegram distingue le cas : « Plancher sans site : 12/50 — la réserve sans site est vide, c'est elle qui bloque ».
- La boucle s'arrête aussi net sur `iaError` (qualification Claude en panne : crédits Anthropic, clé, modèle) — continuer brûlerait le quota looter pour des profils que personne ne trie. L'alerte Telegram porte alors le message d'erreur exact.
- Filet de sécurité si le VPS est HS : un cron Vercel quotidien appelle la même route à 4 h UTC (`vercel.json`) — une seule passe, mais la sélection ne démarre jamais à zéro.

> 💰 **Objectif « 50 prospects/jour » — la vraie contrainte est le quota, pas le code.** Depuis la désactivation d'Apify, looter fait découverte + résolution : ~1 requête par profil résolu, plus les pages de hashtags. Avec le filtre « sans site » et le tri IA, viser 50 qualifiés/jour demande **~100-200 résolutions/jour, soit 3 000-6 000 requêtes/mois**. Le plan gratuit (150/mois) en couvre ~3 %. Il faut souscrire le plan payant d'Instagram Looter sur RapidAPI (~10-15 $/mois pour 15 000 req) — le pipeline se recalibre tout seul (lots de 100 × 10 en parallèle, cf. `tuning()`), rien à changer dans le code. Et garder des crédits Anthropic (console.anthropic.com → Billing) : la qualification tourne sur Haiku, quelques centimes par jour — sans elle, RIEN n'entre jamais dans la sélection.

...
function HomeInner() {
  ...
  const stats = useMemo(() => {
    ...
    const s = { total: filteredByRegion.length, todo: 0, called: 0, positive: 0, negative: 0 };
  ...
  useEffect(() => {
    setPage(1);
  }, [search, filter, regionFilter, metierFilter, villeFilter, openNowOnly, sortBy, pageSize]);
    ...
    <main className="min-h-screen">
      ...
      <div className="px-3 md:px-6 py-3 md:py-4">
        ...
        <div className="text-xs text-neutral-500 mb-2 flex items-center gap-2">
          ...
          {(filter !== "all" || regionFilter !== "all" || metierFilter !== "all" || villeFilter !== "all" || openNowOnly || search) && (
            <button
              onClick={() => {
                setFilter("all"); setMetierFilter("all"); setVilleFilter("all");
                setRegionFilter("all"); setOpenNowOnly(false); setSearch("");
              }}
        ...
        <div className="md:hidden space-y-2">
          ...
          {paginated.length === 0 && (
            <div className="py-12 text-center text-neutral-500 flex flex-col items-center gap-2">
              ...
              <button onClick={() => { setSearch(""); setFilter("all"); setMetierFilter("all"); setVilleFilter("all"); setRegionFilter("all"); setOpenNowOnly(false); }} className="text-xs text-violet-400 hover:text-violet-300">
